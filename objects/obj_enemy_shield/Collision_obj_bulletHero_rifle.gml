...
if chanceOfCrit <= obj_hero.chanceOfCrit_count
{
	if shield <= 0
	{
		minusHp = (obj_gunHero.rifle_hpDamage - ((obj_gunHero.rifle_hpDamage / 100) * armor)) * obj_hero.powerCrit;
		hp -= minusHp;
		start_colorDamage = c_red;
	}
	else
	{
		minusHp = obj_gunHero.rifle_shieldDamage * obj_hero.powerCrit;
		shield -= minusHp;
		start_colorDamage = c_red;
	}
	flash = flashTime;
...
else
{
	if shield <= 0
	{
		minusHp = obj_gunHero.rifle_hpDamage - ((obj_gunHero.rifle_hpDamage / 100) * armor);
		hp -= minusHp;
		start_colorDamage = c_white;
	}
	else
	{
		minusHp = obj_gunHero.rifle_shieldDamage;
		shield -= minusHp;
