...
if chanceOfCrit <= obj_hero.chanceOfCrit_count
{
	if shield <= 0
	{
		minusHp = (obj_gunHero.shotgun_shieldDamage - ((obj_gunHero.shotgun_shieldDamage / 100) * armor)) * obj_hero.powerCrit;
		hp -= minusHp;
		start_colorDamage = c_red;
	}
	else
	{
		minusHp = obj_gunHero.shotgun_shieldDamage * obj_hero.powerCrit;
		shield -= minusHp;
		start_colorDamage = c_red;
	}
	flash = flashTime;
...
else
{
	if shield <= 0
	{
		minusHp = obj_gunHero.shotgun_shieldDamage - ((obj_gunHero.shotgun_shieldDamage / 100) * armor);
		hp -= minusHp;
		start_colorDamage = c_white;
	}
	else
	{
		minusHp = obj_gunHero.shotgun_shieldDamage;
		shield -= minusHp;
