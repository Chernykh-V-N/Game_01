#region изначальные параметры
	gunHero_y = 13;		// объект дробовика относительно объекта гг
	recoil = 0;			// объявляем переменную отдачи
	firing_delay = 0;	// объявляем переменную отдачи

	gun_select = global.gunSelect - 1;	// выбранный тип оружия
	gun_type = global.typeGunSelect;	// выбранный подтип (обычное/модифицированное) оружия
	image_speed = 0;	// чтобы не менялись кадры спрайта оружия
#endregion
...
// 1 тип
	...
	if gun_select = 0 and gun_type = 1		// для начала зарядки модифицированного дробовика
	{
...
#region звуки на каждый тип оружия 
